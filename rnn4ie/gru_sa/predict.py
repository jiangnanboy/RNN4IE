...
from rnn4ie.util.crf_util import get_tags, format_result
...
class Predict():
    ...
    def _predict_ner(self, model, source_vocab, target_vocab, sentence, max_length):
        ...
        with torch.no_grad():
            sentence_output = model(src_tensor, torch.tensor([len(tokenized)]))  # [batch_size, src_len, output_dim]
    ...
    def _predict_crf_ner(self, model, source_vocab, sentence, max_length, tags, tags_map):
        ...
        with torch.no_grad():
            predictions = model(src_tensor, torch.tensor([len(tokenized)]))
    ...
    def predict(self, sentence):
        ...
        if self.model.use_crf:
            predictions = self._predict_crf_ner(self.model, self.source_vocab, sentence, self.max_length, self.tags,
                                                self.tags_map)
        else:
...
if __name__ == '__main__':
    ...
    # load config file
    config = ConfigParser()
    config.read(os.path.join(parent_directory, 'mlrescnn', 'config.cfg'))
    section = config.sections()[0]
    # get path, vocabs of source, target
    source_vocab_path = config.get(section, "source_vocab_path")
    #source_vocab_path = os.path.join(os.path.dirname(os.path.abspath('..')), 'data', source_vocab_path)
    ...
    # load source vocab
    source_vocab = load_vocab(source_vocab_path)
    print("source_vocab size:{}".format(len(source_vocab)))
    # load target vocab
    target_vocab = load_vocab(target_vocab_path)
    print("target_vocab size:{}".format(len(target_vocab)))
    tags = set()
    tags_map = dict()
    kv = target_vocab.stoi
    for k, v in kv.items():
        tags_map[k] = v
        index = k.find('_')
        if index != -1:
    ...
    print('tags_map:{}'.format(tags_map))
    ...
    if use_crf:
        while True:
            ...
            if sentence == '' or sentence == 'exit':
                break
            predictions = predict_crf_ner(model, source_vocab, sentence, max_length, tags, tags_map)
            print("predictions: {}".format(predictions))
    else:
        while True:
            sentence = input("Input sentence:")
            if sentence == '' or sentence == 'exit':
                break
            predictions = predict_ner(model, source_vocab, target_vocab, sentence, max_length)
            print('predictions: {}'.format(predictions))
'''